...
abstract class AttributeProperty<V>(private val attributeName: String? = null) :
    ...
    override fun setValue(thisRef: Element, property: KProperty<*>, value: V) {
        ...
        if (str == null) {
            thisRef.attributes.remove(attributeName ?: property.name)
        } else {
            thisRef.attributes[attributeName ?: property.name] = str
        }
    }
...
): ReadWriteProperty<Element, V> = object : AttributeProperty<V>(attributeName) {
...
fun stringAttributeProperty(attributeName: String? = null): ReadWriteProperty<Element, String?> =
    attributeProp(attributeName = attributeName, valueToString = { v ->
        v
    }, stringToValue = { s -> s })
