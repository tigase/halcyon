...
inline fun Iterable<Item<out HalcyonModule, out Any>>.extendForDependencies(): List<Item<out HalcyonModule, out Any>> {
    ...
    do {
        val toAdd2 = result.flatMap {
            it.provider.requiredModules().filter { c ->
                !result.any { it.provider.TYPE == c.TYPE }
            }.map { Item(it) }
    ...
    x.forEach { node ->
        node.data.provider.requiredModules().flatMap { dep ->
            x.filter { it.data.provider.TYPE == dep.TYPE }
        }.forEach {
            node.after(it)
        }
    }
