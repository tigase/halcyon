 ...
 *
 */
class ColorGenerationModule(override val context: Context) :
    ColorGeneratorEngine(),
    HalcyonModule {
    ...
    companion object : HalcyonModuleProvider<ColorGenerationModule, ColorGenerationModuleConfig> {
        ...
        override fun instance(context: Context): ColorGenerationModule =
            ColorGenerationModule(context)

        override fun configure(
            module: ColorGenerationModule,
            cfg: ColorGenerationModuleConfig.() -> Unit
        ) {
            module.config(cfg)
        }
...
open class ColorGeneratorEngine : ColorGenerationModuleConfig {
    ...
    private fun normalizeHash(hash: Float, min: Float, max: Float) =
        (hash.absoluteValue * (max - min)) + min

    private fun calculateValue(data: String): Float =
        data.encodeToByteArray().sha1().bytes.take(2).reversed()
    ...
    fun checkSettings() {
        ...
        if (saturationMin >
            ...
            throw AssertionError("saturationMin cannot be greater than saturationMax.")
        }
        if (lightnessMin >
            lightnessMax
        ) {
            throw AssertionError("lightnessMin cannot be greater than lightnessMax.")
        }
    }
