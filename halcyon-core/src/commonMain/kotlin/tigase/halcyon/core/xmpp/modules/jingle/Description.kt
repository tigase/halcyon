...
class Description(
    ...
    fun cloneForModify(): Description =
        Description(media, ssrc, emptyList(), null, emptyList(), false, ssrcs, ssrcGroups, emptyList())
    ...
    companion object {
        ...
        val supportedFeatures =
            arrayOf(XMLNS, "urn:xmpp:jingle:apps:rtp:audio", "urn:xmpp:jingle:apps:rtp:video")
        ...
        fun parse(el: Element): Description? {
            if ("description".equals(el.name) && XMLNS.equals(el.xmlns)) {
                ...
                val encryption =
                    el.getFirstChild("encryption")?.children?.map { Encryption.parse(it) }
