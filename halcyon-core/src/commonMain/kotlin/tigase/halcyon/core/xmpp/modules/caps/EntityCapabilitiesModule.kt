...
class EntityCapabilitiesModule(
    ...
    companion object : XmppModuleProvider<EntityCapabilitiesModule, EntityCapabilitiesModuleConfig> {
        ...
        override fun instance(context: Context): EntityCapabilitiesModule =
        ...
        override fun configure(
            module: EntityCapabilitiesModule,
            cfg: EntityCapabilitiesModuleConfig.() -> Unit
        ) = module.cfg()
    ...
    private val log = LoggerFactory.logger(
        "tigase.halcyon.core.xmpp.modules.caps.EntityCapabilitiesModule"
    )
    ...
    inner class CapsNodeDetailsProvider : NodeDetailsProvider {
        ...
        override fun getFeatures(sender: BareJID?, node: String?): List<String> {
            val ver = getVerificationString()
            return if (node ==
                "${this@EntityCapabilitiesModule.node}#$ver"
            ) {
                context.modules.getAvailableFeatures()
                    .toList()
            } else {
                emptyList()
            }
    ...
    private fun getServerNode(): String? =
        streamFeaturesModule.streamFeatures?.getChildrenNS("c", XMLNS)?.let { c ->
        ...
        val frms = forms.sortedBy { it.getFieldByVar("FORM_TYPE")?.fieldValue }.map { form ->
            (
                form.getAllFields().filter { it.fieldName == "FORM_TYPE" }
                    .map { it.fieldValues.joinToString(separator = "<") { it } } +
                    form.getAllFields()
                        .filterNot { it.fieldName == "FORM_TYPE" }.sortedBy { it.fieldName }.map {
                            it.fieldName + "<" +
                                it.fieldValues.sorted().joinToString(separator = "<") { it }
                        }
                )
        }.flatten()
    ...
    private fun storeInfo(node: String, info: DiscoveryModule.Info) {
        ...
        if (!storeInvalid && !isValid) {
            log.warning(
                "JID ${info.jid} provided invalid CAPS verification string. Skipping caching item."
            )
            return
        } else if (!isValid) {
    ...
    fun validateVerificationString(info: DiscoveryModule.Info): Boolean {
        val calculatedVer =
            calculateVer(identities = info.identities, features = info.features, forms = info.forms)
     ...
     */
    fun getServerCapabilities(): Caps? = getServerNode()?.let { serverNode ->
