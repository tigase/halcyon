 ...
 */
class PresenceModule(override val context: Context) :
    XmppModule,
    ...
    companion object : XmppModuleProvider<PresenceModule, PresenceModuleConfig> {
        ...
        override fun configure(module: PresenceModule, cfg: PresenceModuleConfig.() -> Unit) =
            module.cfg()
    }
    ...
    fun sendPresence(
        ...
        show: Show? = null,
        status: String? = null
    ): RequestBuilder<Unit, Presence> = context.request.presence {
    ...
    fun getBestPresenceOf(jid: BareJID): Presence? {
        data class Envelope(val presence: Presence) {
            ...
            val comp: String by lazy {
                "${(500 - presence.priority)}:${100 + presence.typeAndShow().ordinal}"
            }
        }
     ...
     * @param jid JabberID
     */
    fun getPresenceOf(jid: JID): Presence? = store.getPresence(jid)

    /**
     * Returns all known resources of given JabberID.
     * @param jid JabberID
     */
    fun getResources(jid: BareJID): List<JID> = store.getPresences(jid).mapNotNull { p -> p.from }
}
