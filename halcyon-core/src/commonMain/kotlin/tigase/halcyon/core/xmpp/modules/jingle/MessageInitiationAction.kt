...
sealed class MessageInitiationAction(open val id: String, val actionName: String) {
    ...
    class Propose(
        ...
        val descriptions: List<MessageInitiationDescription>,
        val data: List<Element>? = null
    ) : MessageInitiationAction(id, "propose") {
        val media = descriptions.map { Media.valueOf(it.media) }
    }

    class Retract(override val id: String, val reason: TerminateReason?) :
        MessageInitiationAction(id, "retract")
    ...
    class Reject(override val id: String, val reason: TerminateReason?) :
        MessageInitiationAction(id, "reject")
    class Ringing(override val id: String) : MessageInitiationAction(id, "ringing")
    class Finish(override val id: String, val reason: TerminateReason?) :
        MessageInitiationAction(id, "finish")

    companion object {

        fun parse(actionEl: Element): MessageInitiationAction? {
            val id = actionEl.attributes["id"] ?: return null
            val reason = actionEl.getChildrenNS("reason", JingleModule.XMLNS)?.let {
            ...
            when (actionEl.name) {
                ...
                "propose" -> {
                    val descriptions = actionEl.children.mapNotNull {
                        MessageInitiationDescription.parse(it)
                    }
                    if (descriptions.isNotEmpty()) {
                        return Propose(
                            id,
                            descriptions,
                            actionEl.children.filterNot {
                                it.name ==
                                    "description"
                            }
                        )
