...
sealed class SlotRequestException(response: IQ, error: ErrorCondition, description: String?) :
...
class QuotaSlotRequestException(
    response: IQ,
    error: ErrorCondition,
    description: String?,
    val tryAt: Instant?
...
class FileUploadModule(override val context: AbstractHalcyon) :
    HalcyonModule,
    ...
    companion object : HalcyonModuleProvider<FileUploadModule, FileUploadModuleConfig> {
        ...
        override fun instance(context: Context): FileUploadModule =
            FileUploadModule(context as AbstractHalcyon)

        override fun configure(module: FileUploadModule, cfg: FileUploadModuleConfig.() -> Unit) =
            module.cfg()
    }
    ...
    fun requestSlot(
    ...
    ): RequestBuilder<Slot, IQ> = context.request.iq {
        id()
        type = IQType.Get
        to = jid
        "request" {
        ...
        val getUrl = s.getFirstChild("get")?.attributes?.get("url") ?: throw XMPPException(
            ErrorCondition.BadRequest,
            "Missing 'get' element or `url` attribute."
        )
        val putUrl = s.getFirstChild("put")?.attributes?.get("url") ?: throw XMPPException(
            ErrorCondition.BadRequest,
            "Missing 'put' element or `url` attribute."
        )
        val headers =
            s.getFirstChild("put")!!.getChildren("header").associate {
                it.attributes["name"]!! to
                    (it.value ?: "")
            }
                .toMap()
    ...
    private fun createXMPPError(stanza: IQ): XMPPError {
        ...
        if (ftl != null) {
            ...
            return FileTooLargeSlotRequestException(
                stanza,
                error.condition,
                error.message,
                maxFileSize
