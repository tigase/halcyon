...
sealed class BlockingCommandEvent : Event(TYPE) {
     ...
     */
    data class Blocked(val jid: BareJID, val reason: Reason, val text: String?) :
        BlockingCommandEvent()
 ...
 */
class BlockingCommandModule(override val context: Context) :
    AbstractXmppIQModule(
        context,
        TYPE,
        arrayOf(XMLNS),
        Criterion.chain(Criterion.name(IQ.NAME), Criterion.xmlns(XMLNS))
    ),
    BlockingCommandModuleConfig {
    ...
    companion object : XmppModuleProvider<BlockingCommandModule, BlockingCommandModuleConfig> {
        ...
        override fun instance(context: Context): BlockingCommandModule =
            BlockingCommandModule(context)

        override fun configure(
            module: BlockingCommandModule,
            cfg: BlockingCommandModuleConfig.() -> Unit
        ) = module.cfg()
    }
            ...
            items.filter { it.name == "item" }
                .forEach {
                    context.eventBus.fire(
                        BlockingCommandEvent.Unblocked(it.attributes["jid"]!!.toBareJID())
                    )
                }
        }
    }
    ...
    private fun processBlock(block: Element) {
        block.children.filter { it.name == "item" }
            .forEach {
                ...
                context.eventBus.fire(
                    BlockingCommandEvent.Blocked(it.attributes["jid"]!!.toBareJID(), reason, text)
                )
            }
    }
    ...
    private fun createRetrieveResponse(stanza: IQ): List<BareJID> =
        stanza.getChildrenNS("blocklist", XMLNS)?.children?.filter {
            it.name == "item"
        }
    ...
    fun block(
        jid: BareJID,
        reason: Reason = Reason.NotSpecified,
        text: String? = null
