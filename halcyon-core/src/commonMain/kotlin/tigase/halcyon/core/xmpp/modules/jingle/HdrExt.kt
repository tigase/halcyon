...
class HdrExt(val id: String, val uri: String, val senders: Description.Senders) {
    ...
    fun toElement(): Element = element("rtp-hdrext") {
    ...
    companion object {
        ...
        fun parse(el: Element): HdrExt? {
            if ("rtp-hdrext".equals(el.name) &&
                "urn:xmpp:jingle:apps:rtp:rtp-hdrext:0".equals(el.xmlns)
            ) {
                val id = el.attributes["id"] ?: return null
                val uri = el.attributes["uri"] ?: return null
                val senders =
                    el.attributes["senders"]?.let { Description.Senders.valueOf(it) }
                        ?: Description.Senders.Both
