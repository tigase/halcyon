...
class QueryForMessages(
	...
	private fun finish() {
		val rsm = RSM.Result(first = firstRSM?.first, last = lastRSM?.last, index = firstRSM?.index, lastRSM?.count)
	...
	private fun processFin(complete: Boolean, rsm: RSM.Result?) {
		...
		if (rsm?.first == null || ((!ignoreComplete) && complete)) {
			...
			firstRSM = rsm
			log.fine { "Preparing next iteration: receivedMessages=$messageBodiesCount; expected=$expectedBodies" }
			if (lastRSM == null) lastRSM = rsm
			if (messageBodiesCount < expectedBodies) {
				ask(RSM.query {
					if (initialRsm != null && initialRsm.after != null) {
						after(rsm.last ?: "")
					} else before(rsm.first ?: "")
				})
			} else {
	...
	private fun ask(askRsm: RSM.Query?) {
		log.finer { "Preparing MAM request to=$to, node=$node, rsm=$askRsm, with=$with, start=$start, end=$end" }
		val req = mamModule.query(to = to, node = node, rsm = askRsm, with = with, start = start, end = end)
			.response {
			...
			.consume {
				...
				log.fine { "Received message from MAM. hasBody=${it.stanza.body != null}; received=$messageBodiesCount" }
...
fun MAMModule.queryForMessages(
	...
	val qfm = QueryForMessages(this, expectedBodies, to, node, rsm, with, start, end, ignoreComplete)
	qfm.init()
}