 ...
 *
 */
class ChatMarkersModule(override val context: Context) :
    XmppModule,
    StanzaInterceptor,
    ...
    companion object : XmppModuleProvider<ChatMarkersModule, ChatMarkersModuleConfig> {
        ...
        override fun configure(module: ChatMarkersModule, cfg: ChatMarkersModuleConfig.() -> Unit) =
            module.cfg()
    ...
    override fun afterReceive(element: Element): Element {
        ...
        when (command.name) {
            "markable" -> {
                if (autoSendReceived) {
                    element.getOriginID() ?: element.attributes["id"]?.let { id ->
            ...
            Marker.Received.xmppValue -> {
                ...
                if (id !=
                    ...
                    context.eventBus.fire(
                        ChatMarkerEvent(from, id, wrap(element), Marker.Received)
                    )
                }
            }
            ...
            Marker.Acknowledged.xmppValue -> {
                ...
                if (id !=
                    ...
                    context.eventBus.fire(
                        ChatMarkerEvent(from, id, wrap(element), Marker.Acknowledged)
                    )
                }
            }
            ...
            Marker.Displayed.xmppValue -> {
                ...
                if (id !=
                    ...
                    context.eventBus.fire(
                        ChatMarkerEvent(from, id, wrap(element), Marker.Displayed)
                    )
                }
            }
    ...
    private fun isChatMarkerSupported(jid: JID): Boolean {
        val presenceModule =
            context.modules.getModuleOrNull<PresenceModule>(PresenceModule.TYPE) ?: return false
        val capsModule =
            context.modules.getModuleOrNull<EntityCapabilitiesModule>(EntityCapabilitiesModule.TYPE)
                ?: return false
    ...
    override fun beforeSend(element: Element): Element {
        ...
        if (isMarkable(to)) {
            element.add(
                tigase.halcyon.core.xml.element("markable") {
                    xmlns = XMLNS
                }
            )
        }
...
sealed class ChatMarker(
     ...
     * Message has been received by a client.
     */
    class Received(originId: String, sender: JID) :
        ChatMarker(ChatMarkersModule.Marker.Received, originId, sender)
     ...
     * Message has been displayed to a user in a active chat and not in a system notification
     */
    class Displayed(originId: String, sender: JID) :
        ChatMarker(ChatMarkersModule.Marker.Displayed, originId, sender)

    override fun toString(): String =
        "ChatMarker(sender=$sender, marker=$marker, originId='$originId')"
}

/**
 * Add Chat Marker request to stanza.
 */
fun MessageNode.markable() = this.element.add(
    tigase.halcyon.core.xml.element("markable") {
