...
class SDP(val id: String, val contents: List<Content>, val bundle: List<String>) {
    ...
    companion object {
        ...
        fun parse(sdp: String, creator: Content.Creator): Pair<SDP, String>? {
            ...
            val sessionLine = sessionLines.first { it.startsWith("o=") }?.split(" ") ?: return null;
            if (sessionLine.size > 3) {
                val sid = sessionLine[1];
                val id = sessionLine[2];

                val groupParts =
                    sessionLines.firstOrNull { it.startsWith("a=group:BUNDLE ") }?.split(" ") ?: listOf("");
...
fun Content.Companion.parse(sdp: String, creator: Content.Creator): Content? {
    ...
    val payloads = line.subList(3, line.size - 1).map { id ->
        ...
        prefix = "a=fmtp:$id ";
        val params = lines.firstOrNull { it.startsWith(prefix) }?.drop(prefix.length)?.split(";")?.map { it.split("=") }
            ?.map {
                Payload.Parameter(
...
fun Content.toSDP(): String {
    ...
    transports.firstOrNull()?.let {
        ...
        it.fingerprint?.let {
            lines += listOf<String>(
                "a=fingerprint:${it.hash} ${it.value}",
                "a=setup:${it.setup.name}"
            )
        }
    }
