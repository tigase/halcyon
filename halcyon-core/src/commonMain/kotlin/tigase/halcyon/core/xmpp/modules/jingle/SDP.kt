...
object Bundle {
    ...
    fun parse(el: Element): List<String>? =
        if (el.name == "group" && el.xmlns == "urn:xmpp:jingle:apps:grouping:0") {
            el.getChildren("content").mapNotNull {
                it.attributes["name"]
    ...
    fun toElementOrNull(bundle: List<String>?): Element? = bundle?.let {
        if (bundle.isEmpty()) {
            null
        } else {
            toElement(bundle)
    ...
    fun toElement(bundle: List<String>): Element = element("group") {
...
class SDP(val id: String, val contents: List<Content>, val bundle: List<String>) {
    ...
    constructor(contents: List<Content>, bundle: List<String>) : this(
    ...
    fun applyDiff(action: Jingle.ContentAction, diff: SDP): SDP {
        return when (action) {
            Jingle.ContentAction.Add, Jingle.ContentAction.Accept -> SDP(
                id,
            ...
            Jingle.ContentAction.Modify -> {
                ...
                for (diffed in diff.contents) {
                    ...
                    if (idx >= 0) {
                        val newContent =
                            Content(
                                contents[idx].creator,
                                diffed.senders,
                                diffed.name,
                                diffed.description,
                                contents[idx].transports
                            )
    ...
    fun diff(oldSdp: SDP): Map<Jingle.ContentAction, SDP> {
        ...
        oldSdp.contents.filterNot {
            newContentNames.contains(it.name)
        ...
        fun oldContentByName(name: String): Content? = oldSdp.contents.filter {
            it.name == name
        }.firstOrNull()

        contents.mapNotNull { newContent ->
            val oldContent = oldContentByName(newContent.name) ?: return@mapNotNull null
            if ((oldContent.senders ?: Content.Senders.both) == (
                    newContent.senders
            ...
            Content(
                ...
                newContent.senders,
                newContent.name,
                newContent.description?.cloneForModify(),
                emptyList()
            )
        }.takeUnless { it.isEmpty() }?.let {
    ...
    fun toString(sid: String, localRole: Content.Creator, direction: SDPDirection): String {
        val lines: MutableList<String> =
            mutableListOf("v=0", "o=- $sid $id IN IP4 0.0.0.0", "s=-", "t=0 0")
...
fun Content.Companion.parse(
    ...
    val description =
        Description(mediaName, null, payloads, null, encryptions, rtcpMux, ssrcs, ssrcGroups, hdrExts)
...
fun Encryption.toSDP() = sessionParams?.let { "a=crypto:$tag $cryptoSuite $keyParams $it" }
    ?: "a=crypto:$tag $cryptoSuite $keyParams"
