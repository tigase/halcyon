...
class InBandRegistrationModule(context: Context) :
    AbstractXmppModule(
        context = context,
        type = TYPE,
        features = arrayOf(XMLNS),
        criteria = Criterion.chain(
            Criterion.name(IQ.NAME),
            Criterion.xmlns(XMLNS)
        )
    ),
    InBandRegistrationModuleConfig {
    ...
    companion object : XmppModuleProvider<InBandRegistrationModule, InBandRegistrationModuleConfig> {
        ...
        override fun instance(context: Context): InBandRegistrationModule =
            InBandRegistrationModule(context)

        override fun configure(
            module: InBandRegistrationModule,
            cfg: InBandRegistrationModuleConfig.() -> Unit
        ) = module.cfg()
    }
    ...
    fun requestRegistrationForm(toJID: BareJID): RequestBuilder<JabberDataForm, IQ> =
        context.request.iq {
            type = IQType.Get
            to = toJID
            query(XMLNS) {}
    ...
    fun submitRegistrationForm(toJID: BareJID, form: JabberDataForm): RequestBuilder<Unit, IQ> =
        context.request.iq {
            type = IQType.Set
            to = toJID
            query(XMLNS) {
                if ((form.getFieldByVar(REGISTRATION_FORM_TYPE)?.fieldValue) == "plain") {
                    form.getAllFields().filter {
                        it.fieldName in allowedRegistrationFields &&
                            it.fieldName !in listOf(
                                "registered",
                                "instructions"
                            )
                ...
                } else if ((form.getFieldByVar(REGISTRATION_FORM_TYPE)?.fieldValue ?: "form") ==
                    "form"
                ) {
                    form.createSubmitForm().apply {
                        children.firstOrNull {
                            it.name == "field" &&
                                it.attributes["var"] == REGISTRATION_FORM_TYPE
                        }
                            ?.let {
                                remove(it)
                            }
    ...
    fun isAllowed(streamFeatures: Element): Boolean = context.config.registration != null &&
