...
abstract class AbstractJingleSessionManager<S : AbstractJingleSession>(name: String) :
    ...
    private val contactChangeStatusEventHandler = handler<ContactChangeStatusEvent> { event ->
        if (event.lastReceivedPresence.type == PresenceType.Unavailable) {
            val toClose =
                sessions.filter {
                    it.jid == event.presence.from && it.account == event.context.boundJID?.bareJID
                }
    ...
    fun session(context: Context, jid: JID, sid: String?): S? =
        context.boundJID?.bareJID?.let { account ->
            session(account, jid, sid)
        }

    fun session(account: BareJID, jid: JID, sid: String?): S? = lock.withLock {
        sessions.firstOrNull {
            it.account == account &&
                (sid == null || it.sid == sid) &&
                (it.jid == jid || (it.jid.resource == null && it.jid.bareJID == jid.bareJID))
        }
    }
    ...
    override fun messageInitiation(
        context: Context,
        fromJid: JID,
    ...
    override fun sessionInitiated(
        ...
        val media = sdp.contents.map {
            it.description?.media?.let { Media.valueOf(it) }
        }.filterNotNull()

        log.finest(
            "calling session initiated for jid: $jid, sid: $sid, sdp: $media, bundle: $bundle"
        )

        session(context, jid, sid)?.let { session ->
            log.finest(
                "initiating session that already existed for jid: $jid, sid: $sid, sdp: $media, bundle: $bundle"
            )
            session.initiated(contents, bundle)
        } ?: run {
            log.finest(
                "creating an initiating session for jid: $jid, sid: $sid, sdp: $media, bundle: $bundle"
            )
            ...
            reportIncomingCallAction(
                ...
                MessageInitiationAction.Propose(
                    sid,
                    media.map {
                        MessageInitiationDescription("urn:xmpp:jingle:apps:rtp:1", it.name)
                    },
                    null
                )
            )
    ...
    override fun sessionTerminated(
        context: Context,
        jid: JID,
        sid: String,
    ...
    override fun transportInfo(context: Context, jid: JID, sid: String, contents: List<Content>) {
        ...
        for (content in contents) {
            content.transports.flatMap {
                it.candidates
            }.forEach { session.addCandidate(it, content.name) }
        }
    }

    protected fun fireIncomingSessionEvent(
        context: AbstractHalcyon,
        session: S,
        media: List<String>
    ) {
...
class JingleIncomingSessionEvent(
    val session: AbstractJingleSession,
    @Suppress("unused") val media: List<String>
) : Event(TYPE) {
