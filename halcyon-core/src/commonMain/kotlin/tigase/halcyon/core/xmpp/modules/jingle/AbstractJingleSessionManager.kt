...
abstract class AbstractJingleSessionManager<S: AbstractJingleSession>(name: String, private val sessionFactory: SessionFactory<S>): Jingle.SessionManager {
    ...
    private val contactChangeStatusEventHandler: (ContactChangeStatusEvent) -> Unit = { event ->
        if (event.lastReceivedPresence.type == PresenceType.Unavailable) {
            val toClose = sessions.filter { it.jid == event.presence.from && it.account == event.context.config.userJID  };
        ...
        when (event.action) {
            is MessageInitiationAction.Propose -> {
                if (session(account, event.jid, event.action.id) == null) {
                    val session = open(event.context.getModule<JingleModule>(JingleModule.TYPE)!!, account, event.jid, event.action.id, Content.Creator.responder, InitiationType.message);
    ...
    fun open(jingleModule: JingleModule, account: BareJID, jid: JID, sid: String, role: Content.Creator, initiationType: InitiationType): S {
    ...
    protected fun sessionInitiated(event: JingleEvent) {
        ...
        session(account, event.jid, event.sid)?.let { session ->
            ...
            val session = open(event.context.getModule<JingleModule>(JingleModule.TYPE)!!, account, event.jid, event.sid, Content.Creator.responder, InitiationType.iq);
            session.initiated(event.contents, event.bundle);
            fireIncomingSessionEvent(event.context, session, event.contents.map { it.description?.media }.filterNotNull());
        }();
    }
    ...
    interface SessionFactory<S: AbstractJingleSession> {
        fun createSession(jingleSessionManager: AbstractJingleSessionManager<S>, jingleModule: JingleModule, account: BareJID, jid: JID, sid: String, role: Content.Creator, initiationType: InitiationType): S
    }
