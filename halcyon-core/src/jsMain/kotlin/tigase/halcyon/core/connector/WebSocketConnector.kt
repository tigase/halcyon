...
class WebSocketConnectionErrorEvent(@Suppress("unused") val description: String) :
...
class WebSocketConnector(halcyon: Halcyon) : AbstractConnector(halcyon) {
    ...
    override fun createSessionController(): SessionController =
        WebSocketSessionController(halcyon, this)
    ...
    private fun onSocketClose(event: Event): dynamic {
        ...
        if (state ==
            ...
            halcyon.eventBus.fire(
                WebSocketConnectionErrorEvent("Socket unexpectedly disconnected.")
            )
        }
...
private fun List<HostLink>.searchForProtocol(protocolPrefix: String) = this.filter {
    it.rel == "urn:xmpp:alt-connections:websocket"
}.filter { it.href.startsWith(protocolPrefix) }
    .map { it.href }
