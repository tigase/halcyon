...
fun assertContains(expected: Element, actual: Element, message: String? = null) {
    fun check(expected: Element, actual: Element): Boolean {
        ...
        if (!expected.attributes.filter { it.key != "id" }
                .all { e -> actual.attributes[e.key] == e.value }
        ) {
            return false
        }
    ...
    if (!check(expected, actual)) {
        fail(
            messagePrefix(message) +
                "Expected all of ${expected.getAsString()}, actual ${actual.getAsString()}."
        )
    }
}
...
class JabberDataFormTest {
    ...
    private fun createSampleForm(): JabberDataForm = JabberDataForm(
        element("x") {
            xmlns = "jabber:x:data"
            attribute("type", "form")
    ...
    fun testCreateFillSubmit() {
        ...
        assertEquals(
            listOf("reminders", "polls"),
            submit.children.first { it.attributes["var"] == "features" }.children.filter {
                it.name ==
                    "value"
            }
                .mapNotNull { it.value }
