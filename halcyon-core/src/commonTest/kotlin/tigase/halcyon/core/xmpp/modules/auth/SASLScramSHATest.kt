...
class SASLScramSHATest {
    ...
    fun test_first_message_with_authcid() {
        val scram = SASLScramSHA1().apply {
            conceGenerator = { "fyko+d2lbbFgONRv9qkxdawL" }
        }
    ...
    fun test_first_message_with_authcid_equals_localpart() {
        val scram = SASLScramSHA1().also {
            it.conceGenerator = { "fyko+d2lbbFgONRv9qkxdawL" }
        }
    ...
    fun test_messages_sha256() {
        val scram = SASLScramSHA256().apply {
            conceGenerator = { "rOprNGfwEbeRWgbNEkqO" }
        }
        ...
        assertNull(
            scram.evaluateChallenge(
                "v=6rriTRBi23WpRR/wtup+mMhUZUn/dB5nLTJRsjl95G4=".encodeToByteArray().toBase64(),
                DummyHalcyon(),
                configuration,
                context
            )
    ...
    fun test_messages_sha256_invalid_server_signature() {
        val scram = SASLScramSHA256().apply { conceGenerator = { "rOprNGfwEbeRWgbNEkqO" } }
        ...
        assertFailsWith<ClientSaslException>("Fail expected. Given server response is invalid!") {
            scram.evaluateChallenge(
                "v=6rriTRBi23wpRR/wtup+mMhUZUn/dB5nLTJRsjl15G1=".encodeToByteArray().toBase64(),
                DummyHalcyon(),
                configuration,
                context
            )
