...
class MUCModuleTest {
    ...
    fun Join_to_MUC_room() {
        ...
        val room =
            assertNotNull(
                muc.store.findRoom("coven@chat.shakespeare.lit".toBareJID()),
                "Room must be there!"
            )

        assertEquals(State.Joined, room.state, "Room state is not changed")
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.Created
            },
            "RoomCreated event missing"
        )
        ...
        assertEquals(State.NotJoined, room.state, "Room state is not changed")
    ...
    fun Join_to_MUC_room_with_occupant() {
        ...
        assertEquals(State.RequestSent, room.state, "Room state changed")
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.OccupantCame &&
                    event.nickname == "firstwitch"
            },
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.OccupantChangedPresence &&
                    event.nickname == "firstwitch"
            },
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.OccupantLeave &&
                    event.nickname == "firstwitch"
            },
    ...
    fun Join_to_MUC_room_with_occupant_receiving_occupant_kick() {
        ...
        assertEquals(State.Joined, room.state, "Invalid room state")
        assertEquals(1, room.occupants().size)
    }
    ...
    fun Join_to_MUC_room_error() {
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.JoinError &&
                    event.condition == ErrorCondition.JidMalformed
            },
    ...
    fun Join_to_MUC_room_and_be_kicked() {
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.YouLeaved
            },
            "YouLeaved event missing"
        )
        assertEquals(State.NotJoined, room.state, "Room state is not changed")
...
// 				"status"{ attributes["code"] = "110" }
    ...
    fun Join_and_send_message() {
        ...
        assertTrue(
            events.any { event ->
                event is MucRoomEvents.ReceivedMessage &&
                    event.nickname == "thirdwitch" &&
                    event.message.body == "Thrice and once the hedge-pig whined."
            },
    ...
    fun Join_and_receive_direct_message() {
        ...
        halcyon.eventBus.register<MessageReceivedEvent>(MessageReceivedEvent.TYPE) {
            receivedEvent =
                it
        }
    ...
    private fun createRoomAndJoin(muc: MUCModule): Room {
        ...
        halcyon.eventBus.register<MucRoomEvents>(MucRoomEvents.TYPE) {
            when (it) {
                is MucRoomEvents.YouJoined -> println(
                    "You joined to room ${it.room.roomJID} as ${it.nickname}"
                )
                is MucRoomEvents.JoinError -> println(
                    "Oh! You cannot join to room ${it.room.roomJID} because of ${it.condition}"
                )
                else -> {}
            }
        }
    ...
    fun received_mediated_invitation_and_join() {
        ...
        halcyon.eventBus.register<MucRoomEvents>(MucRoomEvents.TYPE) { event ->
            when (event) {
                ...
                is MucRoomEvents.OccupantCame -> println(
                    "Occupant ${event.nickname} came to ${event.room.roomJID}"
                )
                is MucRoomEvents.OccupantLeave -> println(
                    "Occupant ${event.nickname} leaves room ${event.room.roomJID}"
                )
                else -> {}
            }
        }
        ...
        val invitation =
            (
                events.firstOrNull { event ->
                    event is MucEvents.InvitationReceived
                } as MucEvents.InvitationReceived?
                )?.invitation
    ...
    fun received_direct_invitation_and_decline() {
        ...
        assertNull(halcyon.peekLastSend())
    }
