...
class SocketSessionController(halcyon: AbstractHalcyon, private val connector: SocketConnector) :
    AbstractSocketSessionController(
        halcyon,
        "tigase.halcyon.core.connector.socket.SocketSessionController"
    ) {
    ...
    private fun isTLSAvailable(features: Element): Boolean = features.getChildrenNS(
        "starttls",
        ...
        when (event) {
            is SocketConnectionErrorEvent.HostNotFount -> {
                ...
                halcyon.eventBus.fire(
                    SessionController.SessionControllerEvents.ErrorStop("Cannot find server in DNS")
                )
            }

            else -> {
                halcyon.eventBus.fire(
                    SessionController.SessionControllerEvents.ErrorReconnect("Connection error")
                )
            }
        }
    }
