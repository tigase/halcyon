    ...
    fun connect(name: String, port: Int) {
        dispatch_sync(queue) {
            ...
            memScoped {
                ...
                val ip = "${inetaddr[0].toUInt().mod(256u)}.${inetaddr[1].toUInt().mod(256u)}.${inetaddr[2].toUInt().mod(256u)}.${inetaddr[3].toUInt().mod(256u)}";
...
//                            }
    ...
    fun setKeepAlive(timeout: Int, interval: Int) {
        memScoped {
            ...
            if (optval != 0) {
                ret = setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPALIVE, timeout as CValuesRef<*>, sizeOf<IntVar>().toUInt());
                if (ret < 0) {
                    return;
                }
            }
