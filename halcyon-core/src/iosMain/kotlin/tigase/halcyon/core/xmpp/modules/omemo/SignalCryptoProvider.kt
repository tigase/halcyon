...
val signalCryptoProvider: Lazy<signal_crypto_provider> = lazy {
    ...
    provider.encrypt_func = staticCFunction { output, cipher, key, keyLen, iv, ivLen, plaintext, plaintextLen, ctx ->
        encrypt_func(output, cipher, key, keyLen, iv, ivLen, plaintext, plaintextLen, ctx);
    };
    provider.decrypt_func = staticCFunction { output, cipher, key, keyLen, iv, ivLen, ciphertext, ciphertextLen, ctx ->
...
//        output!!.pointed.value = signal_buffer_create(data.toCValues(), outputLen.value);
        ...
        val result = CCCrypt(kCCEncrypt, kCCAlgorithmAES, kCCOptionPKCS7Padding, key, keyLen, iv, plaintext, plaintextLen, data, dataLen, outputLen.ptr);
        if (result != kCCSuccess) {
            return SG_ERR_UNKNOWN;
        }
...
//        output!!.pointed.value = signal_buffer_create(data.toCValues(), outputLen.value);
        ...
        val data = allocArray<UByteVar>(dataLen.toInt());
        val result = CCCrypt(kCCDecrypt, kCCAlgorithmAES, kCCOptionPKCS7Padding, key, keyLen, iv, ciphertext, ciphertextLen, data, dataLen, outputLen.ptr);
        if (result != kCCSuccess) {
            return SG_ERR_UNKNOWN;
        }
