...
import OpenSSL.EVP_CIPHER_CTX_ctrl
import OpenSSL.EVP_CIPHER_CTX_free
import OpenSSL.EVP_CIPHER_CTX_new
import OpenSSL.EVP_CIPHER_CTX_set_padding
import OpenSSL.EVP_CTRL_CCM_SET_TAG
import OpenSSL.EVP_CTRL_GCM_GET_TAG
import OpenSSL.EVP_CTRL_GCM_SET_IVLEN
import OpenSSL.EVP_DecryptFinal_ex
import OpenSSL.EVP_DecryptInit_ex
import OpenSSL.EVP_DecryptUpdate
import OpenSSL.EVP_EncryptFinal_ex
import OpenSSL.EVP_EncryptInit_ex
import OpenSSL.EVP_EncryptUpdate
import OpenSSL.EVP_aes_128_gcm
import OpenSSL.EVP_aes_256_gcm
...
actual object OMEMOEncryptor {
    ...
    private fun retrieveKey(
        ...
        val localKeys = keyElements.filter {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                return DecryptedKey(
                    sessionCipher.decrypt(data = encryptedKey, isPreKey = preKey),
                    preKey
                )
            } catch (e: Exception) {
                ...
                if (log.isLoggable(Level.FINEST)) {
                    log.finest(e, {
                        "failed to decrypt key for " + sessionCipher.address +
                            ", store = " +
                            sessionCipher.store +
                            ", error = " +
                            e.message
    ...
    actual fun decrypt(
        ...
        try {
            ...
            val condition = when (e) {
                is OMEMOException -> {
                    log.warning {
                        "Cannot decrypt message due to condition ${e.condition}: ${stanza.getAsString()}"
                    }
                    e.condition
                }
                is SignalException -> {
                    log.warning {
                        "Cannot decrypt message due to error ${e.error}: ${stanza.getStanzaIDBy(
                            session.localJid
                        )}: ${e.message} : ${stanza.getAsString()}"
                    }
                    when (e.error) {
                    ...
                    log.warning(e) {
                        "Cannot decrypt message due to exception ${e.message} : ${stanza.getAsString()}"
                    }
    ...
    actual fun encrypt(session: OMEMOSession, plaintext: String?): Element {
        ...
        val encrypted = plaintext?.let {
            engine.encrypt(iv, keyData, it.encodeToByteArray())
        }?.also {
...
class AesGcmEngine {
    ...
    fun decrypt(iv: ByteArray, key: ByteArray, payload: ByteArray, tag: ByteArray?): ByteArray? {
        ...
        EVP_DecryptInit_ex(
            ...
            null,
            null,
            key.toUByteArray().toCValues(),
            iv.toUByteArray().toCValues()
        )
        ...
        var encoded = if (tag !=
            ...
            payload.copyOfRange(0, payload.size - 16)
        }
        ...
        val decrypted = memScoped {
            ...
            EVP_DecryptUpdate(
                ...
                output,
                outputLen.ptr,
                encoded.toUByteArray().toCValues(),
                encoded.size
            )
    ...
    fun decrypt(
        iv: ByteArray,
        key: ByteArray,
        chunkProvider: () -> DecryptionStep,
        chunkConsumer: (UByteArray) -> Unit
    ) {
        val ctx = EVP_CIPHER_CTX_new()
        val cipher = if (key.size == 32) {
            EVP_aes_256_gcm()
    ...
    fun encrypt(iv: ByteArray, key: ByteArray, payload: ByteArray): Encrypted {
        ...
        val encrypted = memScoped {
            ...
            EVP_EncryptUpdate(
                ...
                output,
                outputLen.ptr,
                payload.toUByteArray().toCValues(),
                payload.size
            )
                ...
                is EncryptionStep.EndOfInput -> {
                    memScoped {
                        ...
                        if (includeAuthTag) {
                            ...
                            log.finest {
                                "prepared tag data of ${tagData.size}: ${tagData.toHexString()}"
                            }
