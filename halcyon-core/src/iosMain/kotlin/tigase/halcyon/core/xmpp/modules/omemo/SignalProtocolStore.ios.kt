...
import libsignal.SG_SUCCESS
import libsignal.signal_buffer_create
import libsignal.signal_buffer_data
import libsignal.signal_buffer_len
import libsignal.signal_context_create
import libsignal.signal_context_destroy
import libsignal.signal_context_set_crypto_provider
import libsignal.signal_context_set_locking_functions
import libsignal.signal_context_set_log_function
import libsignal.signal_int_list_alloc
import libsignal.signal_int_list_push_back
import libsignal.signal_protocol_address
import libsignal.signal_protocol_identity_key_store
import libsignal.signal_protocol_pre_key_store
import libsignal.signal_protocol_session_store
import libsignal.signal_protocol_signed_pre_key_store
import libsignal.signal_protocol_store_context_create
import libsignal.signal_protocol_store_context_set_identity_key_store
import libsignal.signal_protocol_store_context_set_pre_key_store
import libsignal.signal_protocol_store_context_set_session_store
import libsignal.signal_protocol_store_context_set_signed_pre_key_store
        ...
        fun create(): SignalContext {
            ...
            val context = memScoped {
                ...
                signal_context_set_crypto_provider(
                    ctx.value,
                    signalCryptoProvider.value.readValue()
                )
...
//                    }
    ...
    fun initializeContext(
    ...
    ): CPointer<cnames.structs.signal_protocol_store_context> {
        ...
        return memScoped {
            ...
            sessionStoreCallbacks.get_sub_device_sessions_func =
                staticCFunction(::get_sub_device_sessions_func)
            ...
            sessionStoreCallbacks.delete_all_sessions_func =
                staticCFunction(::delete_all_sessions_func)
            ...
            signedPreKeyStoreCallBacks.load_signed_pre_key =
                staticCFunction(::load_signed_pre_key_func)
            signedPreKeyStoreCallBacks.store_signed_pre_key =
                staticCFunction(::store_signed_pre_key_func)
            signedPreKeyStoreCallBacks.contains_signed_pre_key =
                staticCFunction(::contains_signed_pre_key_func)
            signedPreKeyStoreCallBacks.remove_signed_pre_key =
                staticCFunction(::remove_signed_pre_key_func)
            ...
            signal_protocol_store_context_set_signed_pre_key_store(
                pointer.value,
                signedPreKeyStoreCallBacks.ptr
            )

            val identityKeyStoreCallbacks = alloc<signal_protocol_identity_key_store>()
            identityKeyStoreCallbacks.get_identity_key_pair =
                staticCFunction(::get_identity_key_pair)
            identityKeyStoreCallbacks.get_local_registration_id =
                staticCFunction(::get_local_registration_id)
...
@OptIn(ExperimentalForeignApi::class)
private fun destroy_func(userData: COpaquePointer?) {
    // nothing to do..
}

@OptIn(ExperimentalForeignApi::class)
private fun load_pre_key_func(
    record: CPointer<CPointerVar<signal_buffer>>?,
    preKeyId: UInt,
    userData: COpaquePointer?
): Int {
...
private fun store_pre_key_func(
    ...
    val data = record!!.readBytes(recordLen.toInt())
    userData!!.asStableRef<SignalProtocolStore>().get().storePreKey(
        preKeyId.toInt(),
        PreKeyRecord(data)
    )
    return 1
...
@OptIn(ExperimentalForeignApi::class)
private fun contains_pre_key_func(preKeyId: UInt, userData: COpaquePointer?): Int {
    val result = userData!!.asStableRef<SignalProtocolStore>().get().containsPreKey(
        preKeyId.toInt()
    )
    return if (result) 1 else 0
}
...
@OptIn(ExperimentalForeignApi::class)
private fun load_signed_pre_key_func(
    record: CPointer<CPointerVar<signal_buffer>>?,
    signedPreKeyId: UInt,
    userData: COpaquePointer?
): Int {
    val preKey = userData!!.asStableRef<SignalProtocolStore>().get().loadSignedPreKey(
        signedPreKeyId.toInt()
    )
...
@OptIn(ExperimentalForeignApi::class)
private fun contains_signed_pre_key_func(signedPreKeyId: UInt, userData: COpaquePointer?): Int {
    val result = userData!!.asStableRef<SignalProtocolStore>().get().containsPreKey(
        signedPreKeyId.toInt()
    )
    return if (result) 1 else 0
...
private fun remove_signed_pre_key_func(signedPreKeyId: UInt, userData: COpaquePointer?): Int {
    val result = userData!!.asStableRef<SignalProtocolStore>().get().removePreKey(
        signedPreKeyId.toInt()
    )
    return 1
}
...
@OptIn(ExperimentalForeignApi::class)
private fun get_identity_key_pair(
    publicData: CPointer<CPointerVar<signal_buffer>>?,
    privateData: CPointer<CPointerVar<signal_buffer>>?,
    userData: COpaquePointer?
): Int {
...
@OptIn(ExperimentalForeignApi::class)
private fun get_local_registration_id(
    userData: COpaquePointer?,
    registrationId: CPointer<UIntVarOf<uint32_t>>?
): Int {
    registrationId!!.pointed.value =
        userData!!.asStableRef<SignalProtocolStore>().get().getLocalRegistrationId().toUInt()
    return 1
}

@OptIn(ExperimentalForeignApi::class)
private fun save_identity(
    address: CPointer<signal_protocol_address>?,
    keyData: CPointer<UByteVarOf<uint8_t>>?,
    keyLen: ULong,
    userData: COpaquePointer?
): Int {
    val addr = SignalProtocolAddress.fromPointer(address!!)
...
@OptIn(ExperimentalForeignApi::class)
private fun is_trusted_identity(
    address: CPointer<signal_protocol_address>?,
    keyData: CPointer<UByteVarOf<uint8_t>>?,
    keyLen: ULong,
    userData: COpaquePointer?
    ...
    val addr = SignalProtocolAddress.fromPointer(address!!)
    val identityKey = IdentityKey(ECPublicKeyImpl(keyData!!.readBytes(keyLen.toInt())))
    val result = userData!!.asStableRef<SignalProtocolStore>().get().isTrustedIdentity(
        addr,
        identityKey,
        IdentityKeyStoreDirection.SENDING
    )
    return if (result) 1 else 0
}
...
fun ByteArray.toSignalBuffer(): CPointer<signal_buffer>? =
    signal_buffer_create(this.toUByteArray().toCValues(), this.size.toULong())
